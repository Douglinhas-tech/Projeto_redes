...
{
    public abstract class Jolt : IDisposable
    {
        ...
        public abstract Task UpdateGame();

        #region Body Creation

        protected Body PlayerCreate()
        {
            CapsuleShape shape = new(0.5f, 0.5f);
            using BodyCreationSettings creationSettings = new(shape, new Vector3(3, 20f, 0.0f), Quaternion.Identity, MotionType.Dynamic, Layers.Moving);
            creationSettings.UserData = (uint)GameObjectType.Player;
            ...
            Activation activation = Activation.Activate)
        {
            ...
            using BodyCreationSettings creationSettings = new(shape, position, rotation, motionType, layer);
            creationSettings.UserData = (uint)GameObjectType.None;
            Body body = BodyInterface.CreateBody(creationSettings);
            BodyInterface.AddBody(body.ID, activation);
            _bodies.Add(body);
            return body;
